#!/usr/bin/env python3
"""immortals memory MCP server — exposes the local SQLite store over MCP stdio (AS-021).
...
Run standalone (the orchestrator does this for you):
    python -m immortals.memory.mcp_server --db <path-to-store.db>
"""
...
from immortals.memory.derived import DerivedMemory
from immortals.memory.store import MemoryStore
...
def dispatch(store: MemoryStore, msg: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if method == "initialize":
        ...
        return {"jsonrpc": "2.0", "id": mid, "result": {
            "protocolVersion": ver,
            "capabilities": {"tools": {}},
            "serverInfo": {"name": "immortals-memory", "version": "1.0.0"},
        }}
...
def _resolve_db(argv: list[str]) -> str:
    """The store path: ``--db`` wins, else ``$IMMORTALS_MEMORY_DB``, else a stable default.
    ...
    env_db = os.environ.get("IMMORTALS_MEMORY_DB")
    if env_db:
        return env_db
    default = os.path.join(os.path.expanduser("~"), ".copilot", "immortals", "memory.db")
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        pass
    log_path = os.environ.get("IMMORTALS_MCP_LOG")
